...
//		dispPtr->vdmaConfig.FrameStoreStartAddr[i] = 0x1181f0 + i*0x384000;
//		dispPtr->vdmaConfig.FrameStoreStartAddr[i] = 0xBA6928+(i*0x384000);
		xil_printf("Direccion %d frameStoreStart VGA --> %x\n\r", i, dispPtr->vdmaConfig.FrameStoreStartAddr[i]);
	}
	...
	xil_printf("Canal de lectura --> %x\n\r", &(dispPtr->vdma.ReadChannel));
	Status = XAxiVdma_DmaSetBufferAddr(&(dispPtr->vdma), XAXIVDMA_READ, dispPtr->vdmaConfig.FrameStoreStartAddr);
	xil_printf("Direccion del FrameStoreStartAddr del VGA --> %x\n\r", dispPtr->vdmaConfig.FrameStoreStartAddr);

	if (Status != XST_SUCCESS)
	{
		xil_printf("Read channel set buffer address failed abababalumba balambambu %d\r\n", Status);
		return XST_FAILURE;
	}
...
int DisplayStart2(DisplayCtrl *dispPtr)
{
	int Status;
	u32 vgaReg[5];
	ClkConfig clkReg;
...
**
*/
int DisplayReadInitialize(DisplayCtrl *dispPtr, u16 vdmaId, u32 dispCtrlAddr, int fHdmi, u32 *framePtr[DISPLAY_NUM_FRAMES], u32 stride)
{
	int Status;
	int i;
	...
	dispPtr->vMode = VMODE_1280x720; //Resolucion del video a iniciar /DisplayInitialize original.

	/*
	 * Initialize VDMA driver
	 */
	Config = XAxiVdma_LookupConfig(vdmaId); //Se busca configuracion para el VDMA con el ID --> vdmaId
	...
	Status = XAxiVdma_CfgInitialize(&(dispPtr->vdma), Config, Config->BaseAddress); //Se inicia la configuracion
	xil_printf("La direccion base es: %x \n\r", Config->BaseAddress);
	if (Status != XST_SUCCESS)
	{
...
**
*/
int DisplayReadInitialize2(DisplayCtrl *dispPtr, u16 vdmaId, u32 dispCtrlAddr, int fHdmi, u32 *framePtr[DISPLAY_NUM_FRAMES], u32 stride)
{
	int Status;
	int i;
	XAxiVdma_Config *Config;
	...
	xil_printf("-----------------------------------------------------\n\r");
	xil_printf("-----------------------------------------------------\n\r");
	xil_printf("Terminada de configurar el vgaCtrl\n\r");
	xil_printf("-----------------------------------------------------\n\r");
	xil_printf("-----------------------------------------------------\n\r");
	return XST_SUCCESS;
}
...
//int DisplayWriteInitialize(DisplayCtrl *dispPtr, u16 vdmaId, u32 dispCtrlAddr, int fHdmi, u32 *framePtr[DISPLAY_NUM_FRAMES], u32 stride)
